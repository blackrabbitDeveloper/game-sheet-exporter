...
// ── 빈 셀 (spec.md §6.2 · notation.md §5.2) ──────────────────────────

test('빈 셀의 뜻은 타입이 정하므로 표기가 겹치지 않는다', () => {
  // 생성된 C# CSV 리더가 이 성질에 의존한다. 빈 셀 하나가 타입에 따라 null 이거나
  // 빈 배열이고, 스칼라의 빈 문자열은 시트에서 표현할 수 없으므로 셋이 겹치지 않는다.
  const rows = lines({
    T: [
      ['id', 'a', 'b', 'c'],
      ['int', 'string?', 'int[]', 'int[]?'],
  ...
  assert.equal(rows[0], 'id,a,b,c');
  assert.equal(rows[1], '1,,,', 'null · [] · null 이 모두 맨 빈 필드다');
});

test('스칼라 문자열에서 인용은 값의 일부다', () => {
  // 인용은 배열 원소 문법이다 (notation §5.3). 스칼라 셀의 "" 는 두 글자 값이고
  // 빈 문자열이 아니다 — 그래서 빈 셀과 겹칠 여지가 없다.
  const rows = lines({
    T: [['id', 'a'], ['int', 'string?'], ['', ''], ['1', '""']],
  });

  assert.equal(rows[1], '1,""""""', '두 글자 값이 CSV 규칙으로 이스케이프된다');
});

test('빈 배열은 맨 빈 필드다', () => {
  // T[] 인지 T[]? 인지는 타입이 알려주므로 둘을 구별할 필요가 없다.
  const rows = lines({
    T: [['id', 'a'], ['int', 'int[]'], ['', ''], ['1', '']],
  });
...
test('원소 하나가 빈 문자열인 배열도 표현된다', () => {
  const rows = lines({
    T: [['id', 'a'], ['int', 'string[]'], ['', ''], ['1', '""']],
  });

  // 셀 전체가 인용된 빈 필드다. 타입이 배열이므로 원소 하나로 읽힌다.
  assert.equal(rows[1], '1,""');
});
